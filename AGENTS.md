...
*   **GIF画像の再生成**: `doc/gacha_demo.gif` の再生成 (`npm run generate-gif`) は、UIデザインや演出アニメーションに視覚的な変更があった場合のみ実行してください。不要な再生成はリポジトリサイズを肥大化させ、差分確認を困難にします。なお、GIF生成スクリプトは通常のテスト (`npm run test:e2e`) から分離されています。
